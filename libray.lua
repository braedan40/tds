-------------------------------------------------------------------------------
-- START OF EXTENDED "LIBRARY" EXAMPLE
-------------------------------------------------------------------------------
-- Save this as something like "BetterLibray.lua" in a LocalScript inside
-- StarterPlayerScripts or StarterGui, or load it as a module.

--[[
   Massive UI Library Example
   ~~~~~~~~~~~~~~~~~~~~~~~~~~

   KEY FEATURES:
   1) Create windows and tabs, each with their own layout.
   2) Built-in theming with multiple color presets and dynamic changes.
   3) Automatic tweening and transitions for hover, press, open/close animations.
   4) Builder functions for standard UI elements: Buttons, Toggles, Sliders, Dropdowns,
      ColorPickers, TextBoxes, etc.
   5) Notification manager for app-level messages with fade in/out.
   6) Basic “draggable window” and “resizable window” mechanics.
   7) Example usage at the bottom.

   USAGE:
   local BetterLib = loadstring(game:HttpGet("https://example.com/your-raw-url.lua"))()
   local mainUI = BetterLib:CreateWindow("My Cool UI")
   local homeTab = mainUI:CreateTab("Home")
   local aboutTab = mainUI:CreateTab("About")
   homeTab:CreateButton("Hello", function() print("Hello clicked!") end)
   -- etc...

   NOTE: This library is purely illustrative. Adjust the code to fit your actual project.

   (Inspired by original snippet from [libray.lua](https://raw.githubusercontent.com/braedan40/tds/refs/heads/main/libray.lua))
--]]
...
-------------------------------------------------------------------------------
-- SAMPLE USAGE (Add this at the bottom or call from your script)
-------------------------------------------------------------------------------
--[[
local lib = Library:CreateWindow("TEST")
local testTab = lib:CreateTab("TEST")

lib:CreateButton("TEST", "Press me!", function()
   print("Button pressed!")
end)

lib:CreateToggle("TEST", "Enable Feature?", false, function(newVal)
   print("Toggled: ", newVal)
end)

lib:CreateSlider("TEST", "Volume", 0, 100, 50, function(val)
   print("Slider changed: ", val)
end)

lib:CreateDropdown("TEST", "Choose Option", {"Option A", "Option B", "Option C"}, function(selected)
   print("Dropdown selected: ", selected)
end)

lib:CreateColorPicker("TEST", "Pick Color", Color3.fromRGB(128,128,128), function(color)
   print("Selected color: ", color)
end)

local Notif = lib:GetNotificationManager()
Notif:CreateNotification("Hello from BetterLibray!", 5)

-- Switch theme (example)
lib:SetTheme("Dark")
-- or lib:SetTheme("Light")

-- Enjoy exploring and modifying this expanded library!
--]]
...
-------------------------------------------------------------------------------
-- END OF EXTENDED "LIBRARY" EXAMPLE
-------------------------------------------------------------------------------
